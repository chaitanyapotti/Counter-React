...
const seedWords =
  "young hurdle crash lounge because walk captain cruise garden column receive water";
...
const providerKovan = new HDWalletProvider(
  seedWords,
  "https://kovan.infura.io/v3/3a8a111599ea498c8317dd8a1796cc16",
  0,
  10
);
...
const setupAccounts = async () => {
  ...
  //console.log("Hi2");
  rinkebyContract = await new web3Rinkeby.eth.Contract(
    counterABI,
    rinkebyContractAddress
  );
  rinkebyERCContract = await new web3Rinkeby.eth.Contract(
    tokenABI,
    rinkebyERCContractAddress
  );
  kovanContract = await new web3Kovan.eth.Contract(
    counterABI,
    kovanContractAddress
  );
  kovanERCContract = await new web3Kovan.eth.Contract(
    tokenABI,
    kovanERCContractAddress
  );
...
const createTx = async () => {
  ...
  //App Flow
  //On Landing page, we display user acccount address and network and ask if he is okay with those details
  // if not, we display a message asking him to change those details in metamask and reload
  //A
  //On Landing page, we ask user if he is an initiator or not
  if (yes) {
    //we check if he has already created a tx
  }

  await rinkebyERCContract.methods
    .approve(rinkebyContract.options.address, amountNyto)
    .send({
      from: accountsRinkeby[1]
    });
  console.log("approval 1 done");
  await kovanERCContract.methods
    .approve(kovanContract.options.address, amountSpv)
    .send({
      from: accountsKovan[2]
    });
  console.log("approval 2 done");
  await rinkebyContract.methods
    .createTx(
      true,
      accountsKovan[2],
      encodedSecret,
      rinkebyERCContract.options.address,
      amountNyto
    )
    .send({
      from: accountsRinkeby[1]
  ...
  await kovanContract.methods
    .createTx(
      false,
      accountsRinkeby[1],
      encodedSecret,
      kovanERCContract.options.address,
      amountSpv
    )
    .send({
      from: accountsKovan[2]
  ...
  await rinkebyContract.methods
    .claim(secret, accountsRinkeby[1], amountNyto)
    .send({
      from: accountsKovan[2]
    });
